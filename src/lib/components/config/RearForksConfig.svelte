<script lang="ts">
	import type CC from 'camera-controls';
	import { fly } from 'svelte/transition';
	import { colourMap, partColours } from '$lib/PartColours.svelte';

	let { tab = $bindable(), controls = $bindable() }: { tab: string; controls: CC } = $props();

	let subTab = $state('none');
</script>

{#if subTab == 'none'}
	<!-- Pole Options -->
	<div
		class="fixed bottom-24 z-50 flex w-full justify-center gap-4 font-title uppercase"
		in:fly={{ duration: 300, delay: 200, y: 20 }}
		out:fly={{ duration: 200, y: 20 }}
	>
		<button
			onclick={() => {
				tab = 'default';
				controls?.setPosition(0.1, 0.7, 1.5, true);
				controls?.setTarget(0.2, 0.2, -0.9, true);
			}}
			class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
			aria-label="Back"
		>
			<iconify-icon icon="material-symbols-light:arrow-back"></iconify-icon>
		</button>
		<button
			onclick={() => {
				subTab = 'topTubes';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white px-8 text-[1rem] text-sm font-light duration-300 ease-out hover:-translate-y-1"
			aria-label="Tube"
		>
			Top Tubes
		</button>
		<button
			onclick={() => {
				subTab = 'bottomTubes';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white px-8 text-[1rem] text-sm font-light duration-300 ease-out hover:-translate-y-1"
			aria-label="Tube"
		>
			Bottom Tubes
		</button>
		<button
			onclick={() => {
				subTab = 'bracket';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white px-8 text-[1rem] text-sm font-light duration-300 ease-out hover:-translate-y-1"
			aria-label="Tube"
		>
			Bracket
		</button>
		<button
			onclick={() => {
				subTab = 'arch';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white px-8 text-[1rem] text-sm font-light duration-300 ease-out hover:-translate-y-1"
			aria-label="Arch"
		>
			Arch
		</button>
	</div>
{/if}

{#if subTab == 'topTubes'}
	<div
		class="fixed bottom-24 z-50 flex w-full justify-center gap-4"
		in:fly={{ duration: 300, delay: 200, y: 20 }}
		out:fly={{ duration: 200, y: 20 }}
	>
		<button
			onclick={() => {
				subTab = 'none';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
			aria-label="Back"
		>
			<iconify-icon icon="material-symbols-light:arrow-back"></iconify-icon>
		</button>
		{#each Object.keys(colourMap) as colour}
			<button
				onclick={() => {
					partColours.rearForksTop = colourMap[String(colour)];
					controls?.setPosition(-0.7, 0.7, 0.8, true);
					controls?.setTarget(0, 0.3, -0.2, true);
					tab = 'rearForksConfig';
				}}
				class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
				aria-label="Frame"
			>
				<div class="h-full w-full rounded-full bg-[{colourMap[String(colour)]}]"></div>
			</button>
		{/each}
	</div>
{/if}

{#if subTab == 'bottomTubes'}
	<div
		class="fixed bottom-24 z-50 flex w-full justify-center gap-4"
		in:fly={{ duration: 300, delay: 200, y: 20 }}
		out:fly={{ duration: 200, y: 20 }}
	>
		<button
			onclick={() => {
				subTab = 'none';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
			aria-label="Back"
		>
			<iconify-icon icon="material-symbols-light:arrow-back"></iconify-icon>
		</button>
		{#each Object.keys(colourMap) as colour}
			<button
				onclick={() => {
					partColours.rearForksBottom = colourMap[String(colour)];
					controls?.setPosition(-0.7, 0.7, 0.8, true);
					controls?.setTarget(0, 0.3, -0.2, true);
					tab = 'rearForksConfig';
				}}
				class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
				aria-label="Frame"
			>
				<div class="h-full w-full rounded-full bg-[{colourMap[String(colour)]}]"></div>
			</button>
		{/each}
	</div>
{/if}

{#if subTab == 'bracket'}
	<div
		class="fixed bottom-24 z-50 flex w-full justify-center gap-4"
		in:fly={{ duration: 300, delay: 200, y: 20 }}
		out:fly={{ duration: 200, y: 20 }}
	>
		<button
			onclick={() => {
				subTab = 'none';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
			aria-label="Back"
		>
			<iconify-icon icon="material-symbols-light:arrow-back"></iconify-icon>
		</button>
		{#each Object.keys(colourMap) as colour}
			<button
				onclick={() => {
					partColours.rearForksBracket = colourMap[String(colour)];
					controls?.setPosition(-0.7, 0.7, 0.8, true);
					controls?.setTarget(0, 0.3, -0.2, true);
					tab = 'rearForksConfig';
				}}
				class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
				aria-label="Frame"
			>
				<div class="h-full w-full rounded-full bg-[{colourMap[String(colour)]}]"></div>
			</button>
		{/each}
	</div>
{/if}

{#if subTab == 'arch'}
	<div
		class="fixed bottom-24 z-50 flex w-full justify-center gap-4"
		in:fly={{ duration: 300, delay: 200, y: 20 }}
		out:fly={{ duration: 200, y: 20 }}
	>
		<button
			onclick={() => {
				subTab = 'none';
				controls?.setPosition(-0.7, 0.7, 0.8, true);
				controls?.setTarget(0, 0.3, -0.2, true);
			}}
			class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
			aria-label="Back"
		>
			<iconify-icon icon="material-symbols-light:arrow-back"></iconify-icon>
		</button>
		{#each Object.keys(colourMap) as colour}
			<button
				onclick={() => {
					partColours.rearForksArch = colourMap[String(colour)];
					controls?.setPosition(-0.7, 0.7, 0.8, true);
					controls?.setTarget(0, 0.3, -0.2, true);
					tab = 'rearForksConfig';
				}}
				class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
				aria-label="Frame"
			>
				<div class="h-full w-full rounded-full bg-[{colourMap[String(colour)]}]"></div>
			</button>
		{/each}
	</div>
{/if}

<!-- Rear Forks Config -->
<!-- <div
	class="fixed bottom-24 z-50 flex w-full justify-center gap-4"
	in:fly={{ duration: 300, delay: 200, y: 20 }}
	out:fly={{ duration: 200, y: 20 }}
>
	<button
		onclick={() => {
			tab = 'default';
			controls?.setPosition(0.1, 0.7, 1.5, true);
			controls?.setTarget(0.2, 0.2, -0.9, true);
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Back"
	>
		<iconify-icon icon="material-symbols-light:arrow-back"></iconify-icon>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#120F6C';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#120F6C]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#025c8d';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#025c8d]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#BF1900';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1.25rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#BF1900]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#248048';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#248048]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#E8721C';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#E8721C]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#d56c7a';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#d56c7a]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#5c1970';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#5c1970]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#ffffff';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#ffffff]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#8A8A8A';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#8A8A8A]"></div>
	</button>
	<button
		onclick={() => {
			partColours.rearForks = '#151619';
			controls?.setPosition(-0.7, 0.7, 0.8, true);
			controls?.setTarget(0, 0.3, -0.2, true);
			tab = 'rearForksConfig';
		}}
		class="flex h-10 w-10 -translate-y-0 items-center justify-center rounded-full border border-brandBlue bg-white text-[1rem] font-medium duration-300 ease-out hover:-translate-y-1"
		aria-label="Frame"
	>
		<div class="h-full w-full rounded-full bg-[#151619]"></div>
	</button>
</div> -->
